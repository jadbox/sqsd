	...
	sqsMessage.promise = this.postToWorker(messageBody, sqsMessage)
		.spread((postResult) => {
			...
			if (!(postResult.statusCode < 200 || postResult.statusCode >= 300)) {
				...
				idCache.set(sqsMessage.MessageId, false);
				logger.error({
					MessageId: sqsMessage.MessageId,
					statusCode: postResult.statusCode
...
SQSProcessor.prototype.tick = function() {
    var maxSQSChecks = Math.floor(this.options.maxRequests / this.options.maxMessages);
    maxSQSChecks = maxSQSChecks > 0 ? maxSQSChecks : 1;
    var lenMsgs = this.processingMesages.length;
		if (this.polling > maxSQSChecks || lenMsgs >= maxSQSChecks) {
      logger.warn("Message already recieved: max/polls/prcs: " + maxSQSChecks + "/" + this.polling + "/" + lenMsgs);
			return Promise.resolve()
		}
...
SQSProcessor.prototype.scheduleRun = function() {
	if (this.kill) return this.waitForFinish();
...
SQSProcessor.prototype.start = function() {
  var concurrent = Math.floor(this.options.maxRequests / this.options.maxMessages);
  if(concurrent < 1) concurrent = 1;
	...
	return this.checkWorkerHealth()
		.then((health) => {
			...
			if (this.options.daemonized) {
				this.deferredStop = new Promise(); //new defer();//used to to stop if error happens, should works infinite in OK scenario
